...
module.exports = function( params ) {
	...
	function processInclude( content, filePath, sourceMap ) {
		...
		try {
			...
			this.emit( 'error', new PluginError( 'gulp-include', err, { fileName: filePath } ) );
		}

		if ( errors && errors.length > 0 ) {
			this.emit( 'error', new PluginError( 'gulp-include', '\n' + errors.join( '\n' ), {
				fileName: filePath,
				showStack: false
			} ) );
		}
	...
	function fileNotFoundError( includePath ) {
		if ( hardFail ) {
			...
			console.warn(
				'WARN - no files found matching ' + includePath
			);
		}
	}
